...
function MapView() {
  ...
  const filtered = facilityArr.filter(
    (obj) =>
      searchValue &&
      obj.facility_name.includes(searchValue) &&
      obj.lat !== null,
  );
  ...
  const hospitalMarkers = data.map(({ lat, lng, facility_name }) => {
    ...
    return <Marker longitude={lng} latitude={lat} onClick={handleClick} />;
